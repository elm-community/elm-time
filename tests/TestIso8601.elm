module TestIso8601 exposing (summerTimes, toFromDateISO8601, toFromISO8601)
...
toFromISO8601 =
    let
        ...
        parseEqFlip expectedDateTime input =
            parseEq input expectedDateTime
    ...
    describe "Time.DateTime.{to,from}ISO8601"
        ...
        , fuzz stringFuzzer "fromISO8601 parses offsets correctly" <|
...
stringFuzzer : Fuzzer { hour : Int, minute : Int, negStr : String, separator : String }
stringFuzzer =
    Fuzz.map4 (\h m n s -> { hour = h, minute = m, negStr = n, separator = s })
        (intRange -23 23)
        (intRange 0 59)
    ...
    let
        ...
        parseEq input date () =
            case ( toDate input, date ) of
                ( Err message, _ ) ->
                    Expect.fail (Debug.toString message ++ " in input '" ++ input ++ "'")
